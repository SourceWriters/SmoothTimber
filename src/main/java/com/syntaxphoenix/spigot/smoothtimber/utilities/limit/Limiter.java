...
public final class Limiter {
    ...
    private static final Function<CommandSender, Integer> LIMITED = sender -> {
        ...
        return sender.getEffectivePermissions().stream().filter(attachment -> attachment.getPermission().startsWith("smoothtimber.limit."))
            .filter(PermissionAttachmentInfo::getValue).map(attachment -> Parser.parseInt(attachment.getPermission().substring(19)))
    ...
    public static int getLimit(final CommandSender sender) {
